...
class HabitTestCase(APITestCase):
    ...
    def test_create_habit_validation_error_duration(self):
        ...
        self.assertIn(
            "Время на выполнение полезной привычки не должно превышать 120 секунд", response.json()["duration"]
        )
    ...
    def test_create_habit_validation_error_periodicity(self):
        ...
        self.assertIn(
            "Периодичность выполнения привычки должна быть от 1 до 7 дней", response.json()["periodicity_days"]
        )
